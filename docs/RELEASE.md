...
committed and annotated tag `v2.2.3-port.4` points to that commit. The script
refuses to package if the branch, tag, clean worktree, Java/Gradle version,
dependency presence, automated-test totals or production-JAR contracts differ
...
- `dist/create-deep-seas-forge-1.20.1-2.2.3-port.4.jar` — normal production JAR.
- `release/2.2.3-port.4/` — production/sources JARs, exact tracked-source ZIP,
...
contains those exact build inputs, a verified Git bundle with the port branch
